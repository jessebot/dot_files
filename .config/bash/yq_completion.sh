...
__yq_handle_go_custom_completion()
{
    ...
    requestComp="YQ_ACTIVE_HELP=0 ${words[0]} __completeNoDesc ${args[*]}"
    ...
    if [ $((directive & shellCompDirectiveFilterFileExt)) -ne 0 ]; then
        ...
        if [ -n "$subdir" ]; then
            ...
            __yq_handle_subdirs_in_dir_flag "$subdir"
        else
...
_yq_completion_bash()
{
    last_command="yq_completion_bash"
    ...
    flags+=("--front-matter=")
    two_word_flags+=("--front-matter")
    two_word_flags+=("-f")
    flags+=("--header-preprocess")
    flags+=("--indent=")
    ...
    flags+=("--lua-globals")
    flags+=("--lua-prefix=")
    two_word_flags+=("--lua-prefix")
    flags+=("--lua-suffix=")
    two_word_flags+=("--lua-suffix")
    flags+=("--lua-unquoted")
    flags+=("--no-colors")
    flags+=("-M")
    flags+=("--no-doc")
    ...
    flags+=("-P")
    flags+=("--split-exp=")
    two_word_flags+=("--split-exp")
    two_word_flags+=("-s")
    flags+=("--split-exp-file=")
    two_word_flags+=("--split-exp-file")
    flags+=("--unwrapScalar")
    flags+=("-r")
    flags+=("--verbose")
    flags+=("-v")
    flags+=("--xml-attribute-prefix=")
    two_word_flags+=("--xml-attribute-prefix")
    flags+=("--xml-content-name=")
    two_word_flags+=("--xml-content-name")
    flags+=("--xml-directive-name=")
    two_word_flags+=("--xml-directive-name")
    flags+=("--xml-keep-namespace")
    flags+=("--xml-proc-inst-prefix=")
    two_word_flags+=("--xml-proc-inst-prefix")
    flags+=("--xml-raw-token")
    flags+=("--xml-skip-directives")
    flags+=("--xml-skip-proc-inst")
    flags+=("--xml-strict-mode")
...
_yq_completion_fish()
{
    last_command="yq_completion_fish"
...
_yq_completion_powershell()
{
    last_command="yq_completion_powershell"
...
_yq_completion_zsh()
{
    last_command="yq_completion_zsh"
